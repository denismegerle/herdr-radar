...
// TOML lets the parent be declared after it. Values are never read, except
// to step over a multi-line string: a key binding's command may hold a whole
// TOML snippet, and a header-shaped line inside it is text, not a table.
function claimsTable(text, table) {
  const under = `${table}.`;
  let current = '';
  let quote = null;
  // Whether `rest` closes a multi-line string opened with `quote`. A basic
  // string escapes a quote with a backslash; a literal one cannot.
  const closes = (rest) => (quote === "'''" ? rest.includes("'''") : /(^|[^\\])"""/.test(rest));
  for (const raw of text.split('\n')) {
    const line = raw.trim();
    if (quote) {
      if (closes(line)) quote = null;
      continue;
    }
    // A comment is not a line of TOML at all: it opens no string and names
    // no table, whatever it looks like.
    if (line.startsWith('#')) continue;
    const opened = line.match(/=\s*("""|''')/);
    if (opened) {
      quote = opened[1];
      if (closes(line.slice(opened.index + opened[0].length))) quote = null;
    }
