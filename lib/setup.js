...
function ensure({ force = false } = {}) {
  ...
  // A repo-shared config may contain blocks written on the other OS. Repair
  // installed blocks too, so Windows never runs a Mac `cat` tab-bar command
  // (or vice versa) on its first start. Do not force a reload if unchanged.
  if (report.state === 'installable' || report.state === 'installed') {
