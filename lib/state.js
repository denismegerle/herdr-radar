    ...
    return [
      {
        ...
        //
        // Auto-title's stable task is the most useful single-line title; pi's
        // terminal title often says only "Pi" or the directory. If no task is
        // published, fall back to the pane's display title, then terminal
        // titles. Mirrored panes have no terminal title but do have `title`.
        title: hook.apply(
          'title',
          vendorTitle(
            a.agent ?? '',
            stripVendorPulse(said(tokens.task, a.title, a.terminal_title_stripped, a.terminal_title)),
...
// prefers auto-title's stable task and otherwise uses Herdr's pane/terminal
// title, republished under a state-coloured name.
