...
//
// Answering is not the same as drawing, though. A daemon whose frames have
// stopped still answers — its I/O runs, only its frames do not — and it holds
// the endpoint, so nothing could replace it (#18). It reports how long ago its
// last frame started, and a daemon silent past STALLED_MS is `stalled`: alive,
// not working, and to be replaced. A healthy one frames at least every
// heartbeat (2s) and a slow frame waits out its IPC timeouts in seconds, so
// thirty is far past anything but a stall. A daemon from before this change
// reports no age and is taken at its word rather than killed on upgrade.
const STALLED_MS = 30000;

async function daemonStatus() {
  const reply = await control.request({ cmd: 'ping' });
  if (!reply?.ok) return { state: 'none', pid: null, frameAgeMs: null };
  const age = typeof reply.frame_age_ms === 'number' ? reply.frame_age_ms : null;
  return {
