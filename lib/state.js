...
async function labels(now) {
  ...
  // cached. `workspacesAsync` cannot report failure — it answers `[]` for a
  // timed-out call and for a session with nothing open alike — and a session
  // with nothing open is not a state this runs in: the pane asking is itself
  // in a workspace. So an empty list is a failed read, and committing it would
  // drop every label, redraw every group header as a bare id, and then redraw
  // them all again on the next successful read. The guard on `tabs` was here
  // for exactly that reason; `workspaces` needs it for the same one.
  if (tabs.size > 0 && workspaces.size > 0) Object.assign(cache, { at: now, tabs, workspaces, parents, worktrees });
  return cache;
}
