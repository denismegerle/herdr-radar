...
//
// Both say only where the pane is, which the workspace label on the first
// agent row already says, and neither says which agent is running there — the one thing
...
// Titles that begin with the workspace's own name, when the group's first row
// already says it: the name is written twice and eats the width
...
//
// Only an exact label match followed by a separator is dropped, and only when
// something is left over — `billing · billing` keeps its tail, `api-gateway`
// in a group named `api` is untouched because the boundary is not there. The
// caller passes an empty label in the flat view, where no group label exists.
...
// The first pane of each workspace carries the inline group name. Herdr's
// Agents list has no group headers of its own; grouping only orders entries.
function groupBoundaries(entries) {
  const heads = new Set();
  const seen = new Set();
  for (const entry of entries) {
    if (!entry.workspace || seen.has(entry.workspace)) continue;
    heads.add(entry.pane);
...
async function writeGroups(source, entries, wsLabels, staleWorkspaces = new Set(), tree = {}) {
  ...
  const { heads } = groupBoundaries(entries);
  ...
  const results = await Promise.all(
    entries.map((entry) => {
      ...
      // Orphan worktrees have no real parent pane. Carry their repo name in
      // $group_parent alongside the group label on the first selectable row.
      // Keep the branch corner on the label, without a separate header row.
      ...
      return herdr.reportMetadataAsync(entry.pane, source, {
        ...
        // Only an orphan's head carries it; elsewhere the cell is empty.
        group_parent: orphan && name !== null ? orphanRepo.get(entry.workspace) : null,
        // Clear old spacer tokens: even a visually blank row belongs to the
        // preceding pane's highlight, making the last agent three rows tall.
        gap: null,
      });
    }),
  );
