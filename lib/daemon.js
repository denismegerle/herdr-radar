...
async function start() {
  ...
  const tick = async () => {
    ...
    // Diagnostics only. The bind is the lock, so the pid file gets no vote:
    // it used to, and a pid reused by an unrelated process after a restart
    // could then tell a healthy daemon to exit (#19).
    try {
  ...
  const handlers = {
    // `frame_age_ms` is what lets the watchdog tell a daemon that is alive from
    // one that is still drawing — the first answers pings either way (#18).
    ping: () => ({
      pid: process.pid,
      uptime_ms: Math.round(process.uptime() * 1000),
      frame_age_ms: Math.round(scheduler.frameAgeMs()),
    }),
