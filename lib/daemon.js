...
// How often a daemon confirms it still holds its own endpoint (see tick).
const OWNERSHIP_CHECK_MS = 10000;
...
async function start() {
  ...
  let stopping = false;
  // Monotonic, like the scheduler: the endpoint check below runs every
  // OWNERSHIP_CHECK_MS, and a stepped wall clock must not skip or repeat it.
  const clock = () => performance.now();
  let ownershipCheckedAt = -Infinity;
  ...
  const tick = async () => {
    ...
    // on drawing (lib/control.js serve). The pid file used to settle that by
    // vote, and a pid reused after a restart could win the vote (#19). The
    // endpoint settles it instead: whoever answers it is the daemon, and if
    // that is not this process, this process leaves. On Windows a pipe name
    // cannot be taken over, so this never fires there.
    if (clock() - ownershipCheckedAt > OWNERSHIP_CHECK_MS) {
      ownershipCheckedAt = clock();
      const owner = await control.request({ cmd: 'ping' });
      if (owner?.ok && owner.pid !== process.pid) {
        // Its socket file, its pid file, its activity stamps: leave all of it.
        process.exit(0);
      }
    }

    // Diagnostics only; the endpoint above is what decides who runs.
    try {
      if (Number(fs.readFileSync(state.LOCK(), 'utf8').trim()) !== process.pid) {
