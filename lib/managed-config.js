...
function sidebarBlock(variant) {
  ...
  // Every selectable agent has exactly two rows: a workspace label (on its
  // first pane) + live title, then the stable task. A separate group header or
  // trailing spacer is still part of that pane's selection in Herdr, making
  // the first/last agent highlight three rows. Keep group furniture inline.
  //
  ...
  const agentRow = (vendor) =>
    [
      `[${[
        // Group labels stay on their first pane, but share its first row;
        // Herdr highlights every visible row attached to a selected pane.
        cell('$group_parent', state.idleStale, true),
      ...
      // A stable task from auto-title complements Radar's live activity title.
      // Keep it in the generator: configure/appearance refreshes regenerate
      // both the fallback and every vendor-specific row.
      `[${cell('$task', state.idleStale)}]`,
    ].join(', ');
...
function apply() {
  ...
  const changed = next !== report.text;
  const broken = changed ? checkedWrite(report.file, next, backup(report.file)) : null;
