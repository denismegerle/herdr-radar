...
class Frame {
  ...
  async render(now) {
    ...
    // inline group labels are WRITTEN after that loop: whether a group has
    // gone stale is known only after every pane has a display.
    ...
    for (const entry of entries) {
      ...
      // Herdr indents the continuation task row for us. Every state line is
      // row 1: the head carries the inline group label; later members need
      // an explicit indent. Worktree sessions go one level deeper than their
      // checkout. In priority order, all lines sit at the margin. A split's
      // corner is its offset, so it does not add another level.
  ...
  // Inline group labels and indents, rewritten only when the layout changed.
  // A workspace counts as stale when every session in it is; its label then
  // fades with the sessions instead of looking like an active project.
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines) {
    ...
    // Labels move when the panes, display order, mode or collective staleness
    // changes. Worktree topology changes the branch corners even if the pane
    // list stays the same. A label arriving after the first write also needs
    // to replace its fallback id.
    const fingerprint =
    ...
    // label on the wrong pane, and only a retry fixes that.
